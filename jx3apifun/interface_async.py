...
class ApiInterfaceAsync:
    ...
    @require_token
    async def data_server_sand(self, server: str) -> DataServerSand:
        """
        说明:
            沙盘数据
    ...
    @require_token
    async def data_active_monster(self) -> DataActiveMonster:
        """
        说明:
            本周百战异闻录刷新的首领以及特殊效果。
    ...
    @require_token
    async def data_horse_records(self, server: str = "") -> list[DataHorseRecord]:
        """
        说明:
            客户端的卢刷新记录。
    ...
    @require_token
    async def data_horse_event(self, server: str) -> DataHorseEvent:
        """
        说明:
            客户端马场刷新记录。
    ...
    @require_token
    async def data_watch_record(self, server: str, name: str) -> list[DataWatchRecord]:
        """
        说明:
            烟花赠送与接收的历史记录，不保证遗漏。
    ...
    @require_token
    async def data_watch_statistical(
        self, server: str, name: str, limit: int = 20
    ...
    @require_token
    async def data_watch_collect(self, server: str, num: int = 7) -> list[DataWatchCollect]:
        """
        说明:
            汇总烟花记录。
    ...
    @require_token
    async def data_watch_rank_statistical(
        self,
        server: str,
    ...
    @require_token
    async def data_chat_mixed(self, name: str, text: str) -> DataChatMixed:
        """
        说明:
            智障聊天
    ...
    @require_token
    async def data_music_tencent(self, name: str) -> list[DataMusicTencent]:
        """
        说明:
            搜索腾讯音乐歌曲编号。
    ...
    @require_token
    async def data_music_netease(self, name: str) -> list[DataMusicNetease]:
        """
        说明:
            搜索网易云音乐歌曲编号。
    ...
    @require_token
    async def data_music_kugou(self, name: str) -> list[DataMusicKugou]:
        """
        说明:
            搜索酷狗音乐歌曲编号。
    ...
    @require_token
    async def data_fraud_detail(self, uin: int) -> DataFraudDetail:
        """
        说明:
            搜索贴吧的行骗记录
    ...
    async def data_idiom_solitaire(self, name: str) -> DataIdiomSolitaire:
        """
        说明:
            校对成语并返回相关成语
    ...
    async def data_saohua_random(self) -> DataSaohua:
        """
        说明:
            门派骚话
        """
    ...
    async def data_saohua_content(self) -> DataSaohua:
        """
        说明:
            召唤一条舔狗日记。
        """
    ...
    async def data_sound_converter(
        self,
        appkey: str,
        access: str,
        secret: str,
    ...
    @require_token
    async def data_role_monster(self, server: str, name: str) -> DataRoleMonster:
        """
        说明:
            查看个人百战信息。
